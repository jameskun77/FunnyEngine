...
namespace Funny
{
	...
	{
		for (auto it = mDefaultMaterials.begin(); it != mDefaultMaterials.end(); ++it)
		{
			delete it->second;
		}
	...
	{
		auto found = mDefaultMaterials.find(SID(base));
		if (found != mDefaultMaterials.end())
		{
			Material copy = found->second->Copy();
			Material* mat = new Material(copy);
			mMaterials.push_back(mat); // TODO(Joey): a bit ugly for now, come up with a bettermemory management scheme for materials
			return mat;
		}
		else
		{
	...
	void MaterialLibrary::generateDefaultMaterials()
	{
		...
		mDefaultMaterials[SID("default")] = defaultMat;
		// glass material
		...
		glassMat->SetTexture("TexAlbedo", Funny::Resources::LoadTexture("glass albedo", "textures/glass.png", GL_TEXTURE_2D, GL_RGBA), 0);
		glassMat->SetTexture("TexNormal", Funny::Resources::LoadTexture("glass normal", "textures/pbr/plastic/normal.png"), 1);
		glassMat->SetTexture("TexMetallic", Funny::Resources::LoadTexture("glass metallic", "textures/pbr/plastic/metallic.png"), 2);
		glassMat->SetTexture("TexRoughness", Funny::Resources::LoadTexture("glass roughness", "textures/pbr/plastic/roughness.png"), 3);
		glassMat->SetTexture("TexAO", Funny::Resources::LoadTexture("glass ao", "textures/pbr/plastic/ao.png"), 4);
		glassMat->Blend = true;
		mDefaultMaterials[SID("glass")] = glassMat;
		...
		mDefaultMaterials[SID("alpha blend")] = alphaBlendMaterial;
		...
		mDefaultMaterials[SID("alpha discard")] = alphaDiscardMaterial;
	}
	// --------------------------------------------------------------------------------------------
	void MaterialLibrary::generateInternalMaterials(RenderTarget *gBuffer)
	{
		// post-processing
		Shader* defaultBlitShader = Funny::Resources::LoadShader("blit", "shaders/screen_quad.vs", "shaders/default_blit.fs");
		defaultBlitMaterial = new Material(defaultBlitShader);

		// deferred
		deferredAmbientShader = Funny::Resources::LoadShader("deferred ambient", "shaders/deferred/screen_ambient.vs", "shaders/deferred/ambient.fs");
		deferredIrradianceShader = Funny::Resources::LoadShader("deferred irradiance", "shaders/deferred/ambient_irradience.vs", "shaders/deferred/ambient_irradience.fs");
		deferredDirectionalShader = Funny::Resources::LoadShader("deferred directional", "shaders/deferred/screen_directional.vs", "shaders/deferred/directional.fs");
		deferredPointShader = Funny::Resources::LoadShader("deferred point", "shaders/deferred/point.vs", "shaders/deferred/point.fs");
		...
		// shadows
		dirShadowShader = Funny::Resources::LoadShader("shadow directional", "shaders/shadow_cast.vs", "shaders/shadow_cast.fs");
