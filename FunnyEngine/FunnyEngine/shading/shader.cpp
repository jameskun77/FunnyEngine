...
namespace Funny
{
	...
	void Shader::Load(std::string name, std::string vsCode, std::string fsCode, std::vector<std::string> defines)
	{
		...
		if (defines.size() > 0)
		{
			...
			firstLine = vsCode.substr(0, vsCode.find("\n"));
			if (firstLine.find("#vesion") != std::string::npos)
			{
		...
		if (!status)
		{
			...
			std::cout << "Vertex shader compilation error at: " << name << std::endl;
		...
		if (!status)
		{
			...
			std::cout << "Fragment shader compilation error at: " << name << std::endl;
			std::cout << std::string(log) << std::endl;
		}
		...
		if (!status)
		{
			...
			std::cout << "Shader program linking error: " << std::string(log) << std::endl;
		}
	...
	void Shader::SetMatrixArray(std::string location, int size, glm::mat2* values)
	{

	}

	void Shader::SetMatrixArray(std::string location, int size, glm::mat3* values)
	{

	}

	void Shader::SetMatrixArray(std::string location, int size, glm::mat4* values)
	{
